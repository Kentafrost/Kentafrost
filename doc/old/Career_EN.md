---
layout: default
title: "キャリア詳細"
subtitle: "AWS運用エンジニア → 開発エンジニアへのキャリアチェンジを目指す"
description: "KFrostのキャリア詳細、技術スタック、職歴、個人プロジェクトの詳細情報"
date: 2025-01-01
last_modified_at: 2026-03-07
show_toc: true
permalink: /career/en
---

**🌟 [日本語版](./README.md) 🌟**

<style>
table th {
  background-color: #e8f1fb;
}

table tr:nth-child(even) td {
  background-color: #fafcff;
}

table td,
table th {
  vertical-align: top;
}
</style>

> **AWS Operations Engineer → Transitioning to Software Development Engineer**

## Table of Contents
- [Career Summary](#-career-summary)
- [Work Experience](#-work-experience-details)
- [Technical Skills](#-technical-skills--proficiency)
- [Personal Projects](#-personal-projects--development-achievements)
- [Career Goals](#-career-goals--learning-plan)

---

## Career Summary

| Item | Details |
|------|----------|
| **Total Experience** | {% include career/total-experience-en.txt %} |
| **Specialization** | AWS Operations & Automation, Python Development, Infrastructure Operations |
| **Core Strengths** | - AWS 20+ Services Production Experience<br/>- Python Automation Script Development<br/>- Operations Efficiency & Automation |
| **Current Status** | OSS operations and validation work<br/>Learning technologies are listed in the dedicated learning table below<br/>Aiming for full-stack developer transition |

---

## Work Experience Details
{% include career/work-experience-en.md %}

---

## Technical Skills

{% include career/technology-totals-en.md %}

<br/>

### Cloud & Infrastructure Technologies

<table style="width: 100%; table-layout: auto; border-collapse: collapse;">
  <tr>
    <th align="left" style="width: 15%; word-wrap: break-word;">Category</th>
    <th align="left" style="width: 20%; word-wrap: break-word;">Technology & Language</th>
    <th align="left" style="width: 15%; word-wrap: break-word;">Proficiency</th>
    <th align="left" style="width: 15%; word-wrap: break-word;">Experience</th>
    <th align="left" style="width: 35%; word-wrap: break-word;">Main Achievements & Usage</th>
  </tr>
  
  <!-- Cloud & Infrastructure -->
  <tr>
    <td rowspan="6" align="left"><strong>Cloud<br/>Infrastructure</strong></td>
    <td><strong>AWS Overall</strong></td>
    <td align="left"><strong>⭐⭐⭐⭐⭐</strong></td>
    <td align="left">2 years 4 months</td>
    <td>20+ services operations & automation</td>
  </tr>
  <tr>
    <td>CloudFormation (IaC)</td>
    <td align="left">⭐⭐⭐⭐</td>
    <td align="left">1 year 6 months</td>
    <td>Configuration management & stack automation</td>
  </tr>
  <tr>
    <td>Lambda (Serverless)</td>
    <td align="left">⭐⭐⭐⭐</td>
    <td align="left">2 years 4 months</td>
    <td>PDF processing & notification systems</td>
  </tr>
  <tr>
    <td>EC2 & S3</td>
    <td align="left">⭐⭐⭐⭐</td>
    <td align="left">2 years 4 months</td>
    <td>Instance management & storage operations</td>
  </tr>
  <tr>
    <td>CloudWatch & DynamoDB</td>
    <td align="left">⭐⭐⭐</td>
    <td align="left">1 year 6 months</td>
    <td>Log integration & NoSQL operations</td>
  </tr>
  <tr>
    <td>QuickSight</td>
    <td align="left">⭐⭐</td>
    <td align="left">1 month</td>
    <td>Cost visualization dashboards</td>
  </tr>
</table>

<br/>

### Programming Languages

<table style="width: 100%; table-layout: auto; border-collapse: collapse;">
  <tr>
    <th align="left" style="width: 15%; word-wrap: break-word;">Category</th>
    <th align="left" style="width: 20%; word-wrap: break-word;">Technology & Language</th>
    <th align="left" style="width: 15%; word-wrap: break-word;">Proficiency</th>
    <th align="left" style="width: 15%; word-wrap: break-word;">Experience</th>
    <th align="left" style="width: 35%; word-wrap: break-word;">Main Achievements & Usage</th>
  </tr>
  <tr>
    <td rowspan="3" align="left"><strong>Programming<br/>Languages</strong></td>
    <td><strong>Python</strong></td>
    <td align="left"><strong>⭐⭐⭐⭐⭐</strong></td>
    <td align="left">2 years 4 months</td>
    <td>AWS automation, Lambda, data processing</td>
  </tr>
  <tr>
    <td>VBA (Excel/Access)</td>
    <td align="left">⭐⭐⭐⭐</td>
    <td align="left">2 years 4 months</td>
    <td>Document creation & parameter sheet automation</td>
  </tr>
  <tr>
    <td>PowerShell</td>
    <td align="left">⭐⭐⭐⭐</td>
    <td align="left">2 years 4 months</td>
    <td>Windows management & operations automation</td>
  </tr>
</table>

<br/>

### Frameworks & Libraries

<table style="width: 100%; table-layout: auto; border-collapse: collapse;">
  <tr>
    <th align="left" style="width: 15%; word-wrap: break-word;">Category</th>
    <th align="left" style="width: 20%; word-wrap: break-word;">Technology & Language</th>
    <th align="left" style="width: 15%; word-wrap: break-word;">Proficiency</th>
    <th align="left" style="width: 15%; word-wrap: break-word;">Experience</th>
    <th align="left" style="width: 35%; word-wrap: break-word;">Main Achievements & Usage</th>
  </tr>
  <tr>
    <td rowspan="3" align="left"><strong>Frameworks<br/>Libraries</strong></td>
    <td>boto3 (AWS SDK)</td>
    <td align="left"><strong>⭐⭐⭐⭐⭐</strong></td>
    <td align="left">2 years 4 months</td>
    <td>Core AWS operations & automation technology</td>
  </tr>
  <tr>
    <td>pandas (Data Processing)</td>
    <td align="left">⭐⭐⭐⭐</td>
    <td align="left">2 years</td>
    <td>Data processing & analysis</td>
  </tr>
  <tr>
    <td>matplotlib</td>
    <td align="left">⭐⭐⭐</td>
    <td align="left">1 year</td>
    <td>Data visualization & graph generation</td>
  </tr>
</table>

<br/>

<div style="border: 2px solid #ddd; border-radius: 8px; padding: 15px; margin: 10px 0; background-color: #f9f9f9;">

### Network & Systems

<table style="width: 100%; table-layout: auto; border-collapse: collapse;">
  <tr>
    <th align="left" style="width: 15%; word-wrap: break-word;">Category</th>
    <th align="left" style="width: 20%; word-wrap: break-word;">Technology & Language</th>
    <th align="left" style="width: 15%; word-wrap: break-word;">Proficiency</th>
    <th align="left" style="width: 15%; word-wrap: break-word;">Experience</th>
    <th align="left" style="width: 35%; word-wrap: break-word;">Main Achievements & Usage</th>
  </tr>
  <tr>
    <td rowspan="3" align="left"><strong>Network<br/>Systems</strong></td>
    <td>Incident Response & Troubleshooting</td>
    <td align="left">⭐⭐⭐⭐</td>
    <td align="left">1 year 4 months</td>
    <td>Aviation industry network operations</td>
  </tr>
  <tr>
    <td>Windows Server Operations</td>
    <td align="left">⭐⭐⭐</td>
    <td align="left">3 years 8 months</td>
    <td>EventViewer & system monitoring</td>
  </tr>
  <tr>
    <td>MS Office Suite (Expert)</td>
    <td align="left"><strong>⭐⭐⭐⭐⭐</strong></td>
    <td align="left">3 years 8 months</td>
    <td>Bilingual support & business automation</td>
  </tr>
</table>

</div>

### **Language Skills**

{% include career/language-skills-en.md %}

<br/>

---

## Reference Guide

<div style="background-color: #f6f8fa; border: 2px solid #d1d9e0; border-radius: 8px; padding: 16px; margin: 16px 0;">

### Proficiency Level Definitions
> **Note:** This is a reference guide for understanding the star ratings used throughout the technical skills section above.

<table style="width: 100%; table-layout: auto; border-collapse: collapse; border: 1px solid #d1d9e0;">
  <tr style="background-color: #f1f3f4;">
    <th align="left" style="width: 20%; word-wrap: break-word; padding: 8px; border: 1px solid #d1d9e0;">⭐ Rating</th>
    <th align="left" style="width: 20%; word-wrap: break-word; padding: 8px; border: 1px solid #d1d9e0;">Level</th>
    <th align="left" style="width: 60%; word-wrap: break-word; padding: 8px; border: 1px solid #d1d9e0;">Description</th>
  </tr>
  <tr>
    <td align="left" style="padding: 8px; border: 1px solid #d1d9e0;"><strong>⭐⭐⭐⭐⭐</strong></td>
    <td style="padding: 8px; border: 1px solid #d1d9e0;"><strong>Expert</strong></td>
    <td style="padding: 8px; border: 1px solid #d1d9e0;">Can teach others & propose optimizations</td>
  </tr>
  <tr style="background-color: #f9f9f9;">
    <td align="left" style="padding: 8px; border: 1px solid #d1d9e0;"><strong>⭐⭐⭐⭐</strong></td>
    <td style="padding: 8px; border: 1px solid #d1d9e0;"><strong>Advanced</strong></td>
    <td style="padding: 8px; border: 1px solid #d1d9e0;">Can independently handle complex tasks</td>
  </tr>
  <tr>
    <td align="left" style="padding: 8px; border: 1px solid #d1d9e0;"><strong>⭐⭐⭐</strong></td>
    <td style="padding: 8px; border: 1px solid #d1d9e0;"><strong>Intermediate</strong></td>
    <td style="padding: 8px; border: 1px solid #d1d9e0;">Can independently handle basic tasks</td>
  </tr>
  <tr style="background-color: #f9f9f9;">
    <td align="left" style="padding: 8px; border: 1px solid #d1d9e0;"><strong>⭐⭐</strong></td>
    <td style="padding: 8px; border: 1px solid #d1d9e0;"><strong>Beginner</strong></td>
    <td style="padding: 8px; border: 1px solid #d1d9e0;">Understanding fundamentals & can handle simple tasks</td>
  </tr>
  <tr>
    <td align="left" style="padding: 8px; border: 1px solid #d1d9e0;"><strong>⭐</strong></td>
    <td style="padding: 8px; border: 1px solid #d1d9e0;"><strong>Entry</strong></td>
    <td style="padding: 8px; border: 1px solid #d1d9e0;">Understanding basic concepts & currently learning</td>
  </tr>
</table>

</div>

---

## Personal Projects & Development Achievements

<table style="width: 100%; table-layout: auto; border-collapse: collapse;">
  <tr>
    <th align="left" style="width: 20%; word-wrap: break-word;">Project Name</th>
    <th align="left" style="width: 15%; word-wrap: break-word;">Period</th>
    <th align="left" style="width: 20%; word-wrap: break-word;">Key Technologies</th>
    <th align="left" style="width: 35%; word-wrap: break-word;">Features & Achievements</th>
    <th align="left" style="width: 10%; word-wrap: break-word;">Progress</th>
  </tr>
  
  <tr>
    <td><strong>🗂️ File Organization & Management System</strong></td>
    <td>Apr 2024 – Present</td>
    <td>Python<br/>Windows API<br/>Google Drive API</td>
    <td>• Automated folder organization & classification<br/>• Google Drive integration backup<br/>• Windows Task Scheduler operations</td>
    <td>🟢 **In Production**</td>
  </tr>
  
  <tr>
    <td><strong>Data Visualization & Analysis Tools</strong></td>
    <td>Jun 2024 – Present</td>
    <td>pandas<br/>matplotlib<br/>AWS QuickSight</td>
    <td>• Excel/CSV data processing & analysis<br/>• Graph generation & automated reporting<br/>• QuickSight dashboard integration</td>
    <td>🟢 **In Production**</td>
  </tr>
  
  <tr>
    <td><strong>Web Development Projects</strong></td>
    <td>Aug 2024 – Learning</td>
    <td>React<br/>FastAPI<br/>JavaScript ES6+</td>
    <td>• Personal UI development<br/>• Responsive design implementation<br/>• SPA development</td>
    <td>🟡 **In Development**<br/><a href="https://kentafrost.github.io/ReactApp/">React Demo Page</a></td>
  </tr>
  
  <tr>
    <td><strong>☕ Java Backend Development</strong></td>
    <td>Oct 2024 – Learning</td>
    <td>Spring Boot<br/>Java<br/>Gradle</td>
    <td>• RESTful API design & implementation<br/>• Database integration<br/>• MVC architecture learning</td>
    <td>🟡 **Learning**</td>
  </tr>
</table>

---

## 📜 Certifications & Qualifications

<table style="width: 100%; table-layout: auto; border-collapse: collapse;">
  <tr>
    <th align="left" style="width: 15%; word-wrap: break-word;">Category</th>
    <th align="left" style="width: 30%; word-wrap: break-word;">Certification Name</th>
    <th align="left" style="width: 15%; word-wrap: break-word;">Obtained</th>
    <th align="left" style="width: 40%; word-wrap: break-word;">Related Technologies & Usage</th>
  </tr>
  
  <!-- AWS Certifications -->
  <tr>
    <td rowspan="3" align="left"><strong>AWS<br/>Certifications</strong></td>
    <td align="left"><strong>AWS Certified Solutions Architect – Associate</strong></td>
    <td align="left">2024</td>
    <td>AWS Architecture Design, Construction & Operations</td>
  </tr>
  <tr>
    <td align="left"><strong>AWS Certified DevOps Engineer – Associate</strong></td>
    <td align="left">2025</td>
    <td>CI/CD Pipelines & Infrastructure Automation</td>
  </tr>
  <tr>
    <td align="left"><strong>LinuC Level 1</strong></td>
    <td align="left">2024</td>
    <td>Linux System Administration & Operations Foundation</td>
  </tr>
  
  <!-- Language Certifications -->
  <tr>
    <td align="left"><strong>Language<br/>Certifications</strong></td>
    <td align="left"><strong>TOEIC Listening & Reading Test</strong></td>
    <td align="left">790 points</td>
    <td>English Business Communication Proficiency Certification<br/>(Equivalent to CEFR B2-C1 Level)</td>
  </tr>
</table>

---

## Career Goals & Learning Plan

### Period-Based Career Roadmap

<table style="width: 100%; table-layout: auto; border-collapse: collapse;">
  <tr>
    <th align="left" style="width: 20%; word-wrap: break-word;">Period</th>
    <th align="left" style="width: 25%; word-wrap: break-word;">Main Objectives</th>
    <th align="left" style="width: 35%; word-wrap: break-word;">Specific Initiatives</th>
    <th align="left" style="width: 20%; word-wrap: break-word;">Success Metrics</th>
  </tr>
  
  <tr>
    <td align="left"><strong>2026–2027<br/>(Short-term Goals)</strong></td>
    <td align="left"><strong>Full-Stack<br/>Development Skills</strong></td>
    <td>
      • Complete web app development with React + Spring Boot<br/>
      • Type-safe development with TypeScript<br/>
      • Systematic learning of database & API design<br/>
      • Container operations learning with Docker & Kubernetes<br/>
      • English communications with international teams for OSS management.
    </td>
    <td align="left">Portfolio<br/>Completion & Publication<br/><br/>Technical Certifications</td>
  </tr>
  
  <tr>
    <td align="left"><strong>2028–2029<br/>(Medium & Long-term Goals)</strong></td>
    <td align="left">🏢 **Real-world Development<br/>Experience Acquisition**</td>
    <td>
      • Real web & system development project participation<br/>
      • Team development & agile methodology acquisition<br/>
      • Code & design review experience<br/>
    </td>
    <td align="left">💼 Core Development<br/>Team Member<br/><br/>👥 Junior Mentoring Experience</td>
  </tr>
</table>


### Learning Plan

{% include career/learning-plan-en.md %}

---

### Final Thoughts
• **2 years 4 months AWS operations experience** → Transitioning to full-stack development  
• **Strong commitment** to automation & efficiency  
• **Enthusiastic learner** of new technologies

---

*Last Updated: February 2026 | This README is updated regularly*
