**🌟 [日本語版](./README.md) 🌟**

# 🚀 KFrost - Software/Cloud Engineer

> **AWS Operations Engineer → Transitioning to Software Development Engineer**

## 📁 Table of Contents
- [Career Summary](#-career-summary)
- [Work Experience](#-work-experience-details)
- [Technical Skills](#-technical-skills--proficiency)
- [Personal Projects](#-personal-projects--development-achievements)
- [Career Goals](#-career-goals--learning-plan)

---

## ⚡ Career Summary

| Item | Details |
|------|----------|
| **Total Experience** | 3 years 8 months (May 2022 – Present) |
| **Specialization** | AWS Operations & Automation, Python Development, Infrastructure Operations |
| **Core Strengths** | 🔹 AWS 20+ Services Production Experience<br/>🔹 Python Automation Script Development<br/>🔹 Operations Efficiency & Automation |
| **Current Status** | Learning: React, Java, Spring Boot<br/>Currently performing software validation, aiming for developer position transition |

---

## 📈 Work Experience Details

### 🛠️ Tier-1 Network Incident Response (May 2022 – Aug 2023)

### 🛍️ Retail
- Easy to see tangible results, which helps maintain motivation  
- Direct feedback through customer interaction and sales performance

### 🏗️ Construction
- Work results are visible in the form of physical structures  
- Strong sense of accomplishment from completing projects

### 💻 IT Industry
- Accessible from anywhere via the internet, enabling self-driven learning and experimentation  
- Constantly evolving field that never gets boring  
- Suits my hands-on personality—ideal for self-study, certification, and home-based technical exploration

---

## 📈 Work Experience Details

### 🛠️ Tier-1 Network Incident Response (May 2022 – Aug 2023)

**📋 Basic Information**
- **Duration:** 1 year 4 months
- **Industry:** Aviation Industry (Major Japanese Airline)
- **Employment Type:** SES (System Engineering Services)

**🎯 Main Responsibilities**
- Network incident triage & incident response
- Operations monitoring, troubleshooting, reporting workflow management
- Ping connectivity checks, technician dispatch coordination
- Bilingual email communication (Japanese/English)

---

### ☁️ Cloud Operations Engineer (AWS Focus) (Sep 2023 – Dec 2025)

**📋 Basic Information**
- **Duration:** 2 years 4 months
- **Key Technologies:** AWS, Python, VBA, PowerShell, CloudFormation
- **Scope:** AWS environment operations & automation

**🎯 Major Achievements & Responsibilities**

**📊 Operations Automation & Efficiency**
- Complete automation of document creation & parameter sheet generation using VBA
- AWS cost calculation system construction with QuickSight + Lambda
- EC2 instance usage visualization for cost reduction initiatives

**🏗️ Infrastructure Configuration Management**
- Configuration management of 20+ AWS services using CloudFormation
- Operations & maintenance of DynamoDB, S3, CloudWatch, ECS, Lambda, etc.
- Automated CloudFormation stack creation script development

**🔧 Python Automation Development**
- Automation of configuration data retrieval from 20+ AWS services
- Holiday detection logic implementation (using requests library)
- PDF processing & notification system using Lambda + S3 + SNS
- EventViewer log auto-collection & CloudWatch integration system

---

### 💻 OSS Operations & Development Engineer (Jan 2026 – Present)

**📋 Basic Information**
- **Duration:** 1 month (Ongoing)
- **Key Technologies:** OSS, Java, Kubernetes, AWS, Git/GitHub
- **Scope:** Open Source Software operations, improvement & validation

**🎯 Main Responsibilities**

**🔍 OSS Operations & Validation**
- • OSS operational validation & quality assurance
- • System operations & monitoring
- • Performance optimization & troubleshooting

**📝 Documentation & Knowledge Management**
- • OSS operational documentation creation & updates
- • Best practices documentation
- • Technical knowledge sharing within the organization

**🔧 Code Improvement & Optimization**
- • OSS code adjustments & performance improvements
- • Bug fixes & feature enhancements
- • Code quality improvement & refactoring

**🚀 Future Work Plans**
- • **AWS Service Integration** - Cloud-native architecture construction
- • **Java Development** - Enterprise application development
- • **Kubernetes Operations** - Container orchestration learning & implementation

---

### 💻 Technical Skills & Proficiency Map

<table>
  <tr>
    <th align="center">🏷️ Category</th>
    <th align="center">Technology & Language</th>
    <th align="center">⭐ Proficiency</th>
    <th align="center">⏰ Experience</th>
    <th align="center">📝 Main Achievements & Usage</th>
  </tr>
  
  <!-- ☁️ Cloud & Infrastructure -->
  <tr>
    <td rowspan="8" align="center"><strong>☁️<br/>Cloud<br/>Infrastructure</strong></td>
    <td><strong>AWS Overall</strong></td>
    <td align="center"><strong>⭐⭐⭐⭐⭐</strong></td>
    <td align="center">2 years 4 months</td>
    <td>20+ services operations & automation</td>
  </tr>
  <tr>
    <td>CloudFormation (IaC)</td>
    <td align="center">⭐⭐⭐⭐</td>
    <td align="center">2 years 4 months</td>
    <td>Configuration management & stack automation</td>
  </tr>
  <tr>
    <td>Lambda (Serverless)</td>
    <td align="center">⭐⭐⭐⭐</td>
    <td align="center">2 years 4 months</td>
    <td>PDF processing & notification systems</td>
  </tr>
  <tr>
    <td>EC2 & S3</td>
    <td align="center">⭐⭐⭐⭐</td>
    <td align="center">2 years 4 months</td>
    <td>Instance management & storage operations</td>
  </tr>
  <tr>
    <td>CloudWatch & DynamoDB</td>
    <td align="center">⭐⭐⭐</td>
    <td align="center">2 years 4 months</td>
    <td>Log integration & NoSQL operations</td>
  </tr>
  <tr>
    <td>QuickSight</td>
    <td align="center">⭐⭐⭐</td>
    <td align="center">2 years 4 months</td>
    <td>Cost visualization dashboards</td>
  </tr>
  <tr>
    <td>EKS</td>
    <td align="center">⭐</td>
    <td align="center">Learning</td>
    <td>Currently validating in work projects</td>
  </tr>
  <tr>
    <td>Azure</td>
    <td align="center">⭐</td>
    <td align="center">Planned</td>
    <td>Multi-cloud preparation</td>
  </tr>
  
  <!-- 💻 Programming Languages -->
  <tr>
    <td rowspan="5" align="center"><strong>💻<br/>Programming<br/>Languages</strong></td>
    <td><strong>Python</strong></td>
    <td align="center"><strong>⭐⭐⭐⭐⭐</strong></td>
    <td align="center">2 years 4 months</td>
    <td>AWS automation, Lambda, data processing</td>
  </tr>
  <tr>
    <td>VBA (Excel/Access)</td>
    <td align="center">⭐⭐⭐⭐</td>
    <td align="center">2 years 4 months</td>
    <td>Document creation & parameter sheet automation</td>
  </tr>
  <tr>
    <td>PowerShell</td>
    <td align="center">⭐⭐⭐⭐</td>
    <td align="center">2 years 4 months</td>
    <td>Windows management & operations automation</td>
  </tr>
  <tr>
    <td>JavaScript</td>
    <td align="center">⭐⭐</td>
    <td align="center">Learning</td>
    <td>React development & frontend</td>
  </tr>
  <tr>
    <td>Java</td>
    <td align="center">⭐</td>
    <td align="center">Learning</td>
    <td>Spring Boot development</td>
  </tr>
  
  <!-- 🛠️ Frameworks & Libraries -->
  <tr>
    <td rowspan="5" align="center"><strong>🛠️<br/>Frameworks<br/>Libraries</strong></td>
    <td>boto3 (AWS SDK)</td>
    <td align="center"><strong>⭐⭐⭐⭐⭐</strong></td>
    <td align="center">2 years 4 months</td>
    <td>Core technology for AWS operations & automation</td>
  </tr>
  <tr>
    <td>pandas (Data Processing)</td>
    <td align="center">⭐⭐⭐⭐</td>
    <td align="center">2 years</td>
    <td>Data processing & analysis</td>
  </tr>
  <tr>
    <td>matplotlib</td>
    <td align="center">⭐⭐⭐</td>
    <td align="center">1 year</td>
    <td>Data visualization & graph generation</td>
  </tr>
  <tr>
    <td>React</td>
    <td align="center">⭐⭐</td>
    <td align="center">Learning</td>
    <td>Personal project UI development</td>
  </tr>
  <tr>
    <td>Spring Boot</td>
    <td align="center">⭐</td>
    <td align="center">Learning</td>
    <td>Java backend development</td>
  </tr>
  
  <!-- 🌐 Network & Systems -->
  <tr>
    <td rowspan="3" align="center"><strong>🌐<br/>Network<br/>Systems</strong></td>
    <td>Incident Response & Troubleshooting</td>
    <td align="center">⭐⭐⭐⭐</td>
    <td align="center">1 year 4 months</td>
    <td>Aviation industry network operations</td>
  </tr>
  <tr>
    <td>Windows Server Operations</td>
    <td align="center">⭐⭐⭐</td>
    <td align="center">3 years 8 months</td>
    <td>EventViewer & system monitoring</td>
  </tr>
  <tr>
    <td>MS Office Suite (Expert)</td>
    <td align="center"><strong>⭐⭐⭐⭐⭐</strong></td>
    <td align="center">3 years 8 months</td>
    <td>Bilingual support & business automation</td>
  </tr>
</table>

### 🏆 Proficiency Level Definitions
- **⭐⭐⭐⭐⭐** **Expert** - Can teach others & propose optimizations
- **⭐⭐⭐⭐** **Advanced** - Can independently handle complex tasks
- **⭐⭐⭐** **Intermediate** - Can independently handle basic tasks
- **⭐⭐** **Beginner** - Understanding fundamentals & can handle simple tasks
- **⭐** **Entry** - Understanding basic concepts & currently learning



## 🚀 Personal Projects & Development Achievements

<table>
  <tr>
    <th align="left">📂 Project Name</th>
    <th align="center">⏰ Period</th>
    <th align="center">💻 Key Technologies</th>
    <th align="left">📝 Features & Achievements</th>
    <th align="center">📊 Progress</th>
  </tr>
  
  <!-- Automation Tools Development -->
  <tr>
    <td rowspan="2"><strong>🗂️ File Organization &<br/>Management System</strong></td>
    <td align="center">Apr 2024 – Present</td>
    <td align="center">Python<br/>Windows API<br/>Google Drive API</td>
    <td>• Automated folder organization & classification<br/>• Google Drive integration backup<br/>• Windows Task Scheduler operations</td>
    <td align="center">🟢 **In Production**</td>
  </tr>
  <tr>
    <td><strong>📊 Data Visualization &<br/>Analysis Tools</strong></td>
    <td align="center">Jun 2024 – Present</td>
    <td align="center">pandas<br/>matplotlib<br/>AWS QuickSight</td>
    <td>• Excel/CSV data processing & analysis<br/>• Graph generation & automated reporting<br/>• QuickSight dashboard integration</td>
    <td align="center">🟢 **In Production**</td>
  </tr>
  
  <!-- Web Development Projects -->
  <tr>
    <td rowspan="2"><strong>🌐 Web Development<br/>Projects</strong></td>
    <td align="center">Aug 2024 – Learning</td>
    <td align="center">React<br/>FastAPI<br/>JavaScript ES6+</td>
    <td>• Personal & public UI development<br/>• Responsive design implementation<br/>• SPA (Single Page Application) development</td>
    <td align="center">🟡 **In Development** <br />
      <a href="https://kentafrost.github.io/ReactApp/">React Demo Page</a>
    </td>
  </tr>
  <tr>
    <td><strong>☕ Java Backend<br/>Development</strong></td>
    <td align="center">Oct 2024 – Learning</td>
    <td align="center">Spring Boot<br/>Java<br/>Gradle</td>
    <td>• RESTful API design & implementation<br/>• Database integration<br/>• MVC architecture learning</td>
    <td align="center">🟡 **Learning**</td>
  </tr>
</table>

---

## 🎯 Career Goals & Learning Plan

### 📅 **Period-Based Career Roadmap**

<table>
  <tr>
    <th align="center">⏰ Period</th>
    <th align="center">🎯 Main Objectives</th>
    <th align="left">📋 Specific Initiatives</th>
    <th align="center">📊 Success Metrics</th>
  </tr>
  
  <tr>
    <td align="center"><strong>2026–2027<br/>(Short-term Goals)</strong></td>
    <td align="center">🚀 **Full-Stack<br/>Development Skills**</td>
    <td>
      • Complete web application development with React + Spring Boot<br/>
      • Type-safe development with TypeScript introduction<br/>
      • Systematic learning of database design & API design<br/>
      • Container operations learning with Docker & Kubernetes
    </td>
    <td align="center">Portfolio<br/>Completion & Publication<br/><br/>🏆 Technical Certifications</td>
  </tr>
  
  <tr>
    <td align="center"><strong>2028–2029<br/>(Medium & Long-term Goals)</strong></td>
    <td align="center">🏢 **Real-world Development<br/>Experience Acquisition**</td>
    <td>
      • Participation in real web development & system development projects<br/>
      • Team development & agile development methodology acquisition<br/>
      • Code review & design review experience<br/>
      • Leadership & mentoring skill development
    </td>
    <td align="center">💼 Core Development<br/>Team Member<br/><br/>👥 Junior Mentoring Experience</td>
  </tr>
</table>


### 📚 **Learning Technology Roadmap**

<table>
  <tr>
    <th align="center">🔥 Priority</th>
    <th align="center">📂 Technology Domain</th>
    <th align="left">💻 Technologies & Frameworks</th>
    <th align="center">⏰ Learning Timeline</th>
    <th align="left">📝 Learning Purpose & Applications</th>
  </tr>
  
  <!-- High Priority -->
  <tr>
    <td rowspan="6" align="center"><strong>🔥<br/>Highest Priority<br/>(2026)</strong></td>
    <td align="center"><strong>Languages & Fundamentals</strong></td>
    <td>TypeScript, Java (Spring Boot)</td>
    <td align="center">First half of 2026</td>
    <td>Type-safe large-scale development & enterprise development</td>
  </tr>
  <tr>
    <td align="center"><strong>Frontend</strong></td>
    <td>React (Advanced), Vue.js (New)</td>
    <td align="center">Throughout 2026</td>
    <td>Modern SPA development & framework comparison learning</td>
  </tr>
  <tr>
    <td align="center"><strong>Databases</strong></td>
    <td>MongoDB, PostgreSQL, Redis</td>
    <td align="center">Throughout 2026</td>
    <td>NoSQL, Relational DB & caching</td>
  </tr>
  <tr>
    <td align="center"><strong>Container & Orchestration</strong></td>
    <td>Docker, Kubernetes, Docker Compose</td>
    <td align="center">Second half of 2026</td>
    <td>Containerization, orchestration & DevOps</td>
  </tr>
  <tr>
    <td align="center"><strong>CI/CD & Automation</strong></td>
    <td>GitHub Actions, Jenkins</td>
    <td align="center">Second half of 2026</td>
    <td>Automation pipelines & continuous delivery</td>
  </tr>
  <tr>
    <td align="center"><strong>Cloud Expansion</strong></td>
    <td>Azure (Basic~Intermediate), GCP (Basic)</td>
    <td align="center">Second half of 2026</td>
    <td>Multi-cloud support & option expansion</td>
  </tr>
  
  <!-- Medium Priority -->
  <tr>
    <td rowspan="2" align="center"><strong>⚡<br/>Important<br/>(2027)</strong></td>
    <td align="center"><strong>Testing Technologies</strong></td>
    <td>Jest, JUnit, Selenium, Cypress</td>
    <td align="center">First half of 2027</td>
    <td>Unit, integration, E2E testing & quality assurance</td>
  </tr>
  <tr>
    <td align="center"><strong>IaC & Monitoring</strong></td>
    <td>Terraform, Ansible, Prometheus</td>
    <td align="center">Second half of 2027</td>
    <td>Infrastructure as Code & operations monitoring automation</td>
  </tr>
  
  <!-- Future Priority -->
  <tr>
    <td rowspan="3" align="center"><strong>📅<br/>Long-term<br/>(2028+)</strong></td>
    <td align="center"><strong>Mobile Development</strong></td>
    <td>React Native, Flutter, Swift</td>
    <td align="center">2028 onwards</td>
    <td>Cross-platform & native app development</td>
  </tr>
  <tr>
    <td align="center"><strong>AI/ML</strong></td>
    <td>TensorFlow, PyTorch, scikit-learn</td>
    <td align="center">2028 onwards</td>
    <td>Machine learning, data science & AI utilization</td>
  </tr>
  <tr>
    <td align="center"><strong>Emerging Tech</strong></td>
    <td>Solidity, Web3, GraphQL</td>
    <td align="center">As needed</td>
    <td>Blockchain, distributed systems & advanced APIs</td>
  </tr>
</table>

---

### 💡 **Final Thoughts**
**Leveraging 2 years and 4 months of AWS cloud operations experience, I continue to grow as a full-stack development engineer.**  
**With a strong commitment to automation & efficiency and enthusiasm for learning new technologies, I will contribute to the team!** ⚡

---

*Last Updated: February 2026 | This README is updated regularly*
